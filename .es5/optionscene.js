...
var OptionLayer = cc.Layer.extend({
    ...
    ctor: function ctor() {
        ...
        this.selects = [];
        for (i = 5; i < 9; i++) {
            var select = new CheckLabel(res.selectnormalbg, res.selectbg, res.selectbg, "Arial", 14, cc.color(0, 0, 0));
            select.setText(i);
    ...
    onPlay: function onPlay(sender, type) {
    ...
    onSelect: function onSelect(sender, type) {
        if (type != ccui.Widget.TOUCH_ENDED) return;

        for (i = 0; i < this.selects.length; i++) {
            if (sender == this.selects[i]) {} else {
...
var OptionScene = cc.Scene.extend({
    onEnter: function onEnter() {
        ...
        if (optionSceneInitialized) return;
