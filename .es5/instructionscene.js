...
var IndicatorLayer = cc.Layer.extend({
    ...
    setIndex: function setIndex(idx) {
        ...
        if (curIndex == 0) this.prevBtn.setVisible(false);else this.prevBtn.setVisible(true);

        if (curIndex == indicatorValues.length - 1) this.nextBtn.setVisible(false);else this.nextBtn.setVisible(true);

        if (this.getParent()) this.getParent().imageLayer.setIndex(curIndex);
    },
    onPrev: function onPrev(sender, type) {
        if (type != ccui.Widget.TOUCH_ENDED) return;
        if (curIndex > 0) this.setIndex(curIndex - 1);
    },
    onNext: function onNext(sender, type) {
        if (type != ccui.Widget.TOUCH_ENDED) return;
        if (curIndex < indicatorValues.length - 1) this.setIndex(curIndex + 1);
    }
});
...
var InstructionScene = cc.Scene.extend({
    onEnter: function onEnter() {
        ...
        if (instructionSceneInitialized) return;
